/**
 * Pure analytics-overview math (no DB, no env).
 *
 * Turns a list of published snapshot rows into the six Resumen sections
 * (basket, availability, movers, categories, promotions, competitiveness).
 * The HTTP/DB layer lives in `analyticsOverview.ts` so unit tests can exercise
 * the algorithm without importing the side-effecting Supabase client.
 *
 * The math mirrors the dashboard mock (`src/lib/mock-analytics.ts`) one-to-one
 * for shape, units, base-100 conventions and sort orders. Calendar bucketing
...
/**
 * Apply defaults + the client-data floor to a requested window.
 * `today` / `floorDate` are `YYYY-MM-DD` in Buenos Aires.
 */
export function resolveAnalyticsWindow(
  params: Pick<AnalyticsOverviewParams, 'from' | 'to' | 'window'>,
  today: string,
  floorDate: string,
): { from: string; to: string; windowDays: number } {
  const windowDays = params.window && params.window > 0 ? Math.floor(params.window) : 30;
  const to = params.to ?? today;
  let from = params.from ?? addDays(today, -119);
  if (from < floorDate) from = floorDate;
...
/** Normalized published snapshot used by the panel builders. */
...
/**
 * Roll published snapshot rows into the six analytics sections.
 *
 * `range` is the requested window; the returned `range` is clamped to days
 * that actually have data so base-100 indices start on a real basket day
 * (and "today", usually still pending review, doesn't produce a trailing empty
 * day). Returns a zeroed but well-shaped payload when the window is empty.
 *
 * `autoReduceCompetitiveness` drops the sparsest chains until a shared basket
 * appears — used for the default "all chains" view. An explicit supermarket
 * filter should pass `false` so the caller's set is honored literally.
 */
export function assembleAnalyticsOverview(
  rows: SnapRow[],
  range: { from: string; to: string },
  windowDays: number,
  autoReduceCompetitiveness = true,
): AnalyticsOverview {
  if (rows.length === 0) {
    return emptyOverview(range.from, range.to, windowDays);
  }

  let minDate = range.to;
  let maxDate = range.from;
  for (const r of rows) {
  ...
  if (minDate > maxDate) return emptyOverview(range.from, range.to, windowDays);
  ...
  return {
    ...
    competitiveness: buildCompetitiveness(cells, days, autoReduceCompetitiveness),
  };
}
