-- =============================================================================
-- Require an EAN in the client views.
--
-- A product with no EAN (p.ean NULL/'') is useless to the client — they key
-- everything by barcode — and shows up as a blank-EAN, price-less row. This
-- happened with MercadoLibre listings (MLA… URLs) that don't expose a barcode,
-- so they were ingested EAN-less and leaked into the export.
--
-- Guard both client views so an EAN-less product can NEVER reach the export or
-- the API: add `AND COALESCE(p.ean, '') <> ''`. Everything else is byte-for-byte
-- the previous definition (client_base = migration 012; client_base_preview =
-- migration 020). Data is retained in the DB; heal an EAN onto the mapping and
...
-- -----------------------------------------------------------------------------
-- 1. client_base (published-only) — migration 012 body + EAN guard.
-- -----------------------------------------------------------------------------
...
-- -----------------------------------------------------------------------------
-- 2. client_base_preview (incl. pending_review) — migration 020 body + EAN guard.
-- -----------------------------------------------------------------------------
