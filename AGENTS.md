...
- `**docs/BANK_PROMOS.md**` — design + build doc (**IMPLEMENTED** for Naranja X) for a **new isolated module** that scrapes **bank / credit-card promotions** weekly into its own tables (`promo_providers`, `promotions`, `promotion_snapshots`, migration 029) and serves a **separate dashboard** under `/v1/promos/*`, gated by a new `promos` API-key scope **plus an explicit route guard** so the existing supermarket (full-access) client can't see it. Naranja X is a direct public-JSON provider (paged `POST /binder/filter` per category — no token, no Playwright). Mirrors the revistas/instore pattern (`src/promos/`: types/config/normalize/naranjax/registry/store/pipeline; `npm run promos:run` / `promos:doctor`; weekly `PROMOS_CRON`, gated by `PROMOS_ENABLED`). Read before touching the promotions module.
